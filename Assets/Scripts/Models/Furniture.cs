...
public class Furniture : IXmlSerializable {
	...
	/*
	public Furniture(string objectName, float movementCost = 1f, int width = 1, int height = 1, bool linksToNeighbour = false, bool roomEnclosure = false) {
	...
	*/
	...
	static public Furniture PlaceInstance(Furniture proto, Tile tile) {
		...
		if (obj.linksToNeighbour) {
			...
			if (t != null && t.furniture != null && t.furniture.cbOnChanged != null && t.furniture.objectName == obj.objectName) {
	...
	public void ReadXmlPrototype(XmlReader reader_parent) {
		...
		objectName = reader_parent.GetAttribute("objectName");
					...
					while (invs_reader.Read()) {
						if(invs_reader.Name == "Inventory") {
							invs.Add(new Inventory(
								invs_reader.GetAttribute("objecName"), 
								int.Parse(invs_reader.GetAttribute("amount")), 0));
						}
					}
					
					Job j = new Job(null, objectName, FurnitureActions.JobComplete_FurnitureBuilding, jobTime, invs.ToArray());

					World.current.SetFurnitureJobPrototypes(j, this);
					break;
				case "OnUpdate":
	...
	public void ReadXmlParams(XmlReader reader) {
		...
		Debug.Log("I purposely broke this to get this piece of shit working again");
