...
public class World : IXmlSerializable {
	...
	public void SetFurnitureJobPrototypes(Job j, Furniture f) {
		furnitureJobPrototypes[f.objectName] = j;
	}
	...
	void CreateFurniturePrototypes() {
		...
		// TODO: Probably we should be getting passed a StreamIO handle or the raw
		...
		if (reader.ReadToDescendant("Furnitures")) {
			if (reader.ReadToDescendant("Furniture")) {
				do {
					...
					furniturePrototypes[furn.objectName] = furn;
	...
	*/
	...
	public Furniture PlaceFurniture(string objectName, Tile t, bool doRoomFloodFill = true) {
		//Debug.Log("PlaceInstalledObject");
		// TODO: This function assumes 1x1 tiles -- change this later!

		if (furniturePrototypes.ContainsKey(objectName) == false) {
			Debug.LogError("furniturePrototypes doesn't contain a proto for key: " + objectName);
			return null;
		}

		Furniture furn = Furniture.PlaceInstance(furniturePrototypes[objectName], t);
	...
	public Furniture GetFurniturePrototype(string objectName) {
		if (furniturePrototypes.ContainsKey(objectName) == false) {
			Debug.LogError("No furniture with type: " + objectName);
			return null;
		}

		return furniturePrototypes[objectName];
	}
	...
	void ReadXml_Furnitures(XmlReader reader) {
		...
		if (reader.ReadToDescendant("Furniture")) {
			do {
				...
				Furniture furn = PlaceFurniture(reader.GetAttribute("objectName"), tiles[x, y], false);
				furn.ReadXml(reader);
			} while (reader.ReadToNextSibling("Furniture"));
